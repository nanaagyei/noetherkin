...
export function currentTask(root: string, rt: Runtime = runtime): ObjectValue | undefined {
  const files = taskFiles(root, rt);
  if (!files.length) return undefined;
  return record(root, `work/${files.at(-1)}`, rt);
}
...
export function inspectCatalogProject(root: string, projectId: string, sourceInput: string, rt: Runtime = runtime): { source_path: string; revision: string } {
  ...
  if (project.support.task_packs.includes('pet-type-integrity')) assertTaskCompatibleSource(source);
...
function assertTaskCompatible(root: string, rt: Runtime): void {
  const selection = record(root, 'current-project.yaml', rt);
  const source = safePath(root, selection.source_path, rt);
  assertTaskCompatibleSource(source);
}
export function assignTask(root: string, rt: Runtime = runtime): ObjectValue {
  checkMap(root, rt); assertTaskCompatible(root, rt);
  const prior = currentTask(root, rt); if (prior) return { outcome: 'no-change', task_id: prior.id, status: prior.status };
  const template = taskTemplate(); const id = rt.id('TASK'); const at = rt.now(); const teamLead = actor(root, 'team-lead', rt);
  const { compatibility: _compatibility, next_task_preview: _preview, version: _version, id: _templateId, ...assignment } = template;
  const task = { ...common, id, created_at: at, author: teamLead, ...assignment, status: 'assigned', assigned_by: teamLead, assistance_history: [], blocked_reason: null, completion_evidence_ids: [], validation: [], transitions: [{ from: 'absent', to: 'backlog', at, actor: teamLead, reason: 'Instantiated the compatible curated Phase 6 training task.' }, { from: 'backlog', to: 'assigned', at, actor: teamLead, reason: 'Team lead assigned the learner-local training task.' }], work_artifact: null, design_artifact: null, design_assessment_id: null };
  const checkedMap = checkMap(root, rt);
  return { outcome: 'success', ...publish(root, 'team-lead', 'assign-task', id, { template: 'pet-type-integrity', map: checkedMap.artifact }, new Map([[statePath(`work/${id}.yaml`), encode(task)]]), [id], rt), task_id: id, investigation_prompt: 'Trace the pet create/update path and predict what happens when the requested pet type cannot be resolved. Do not change code until your design is reviewed.' };
}
...
export function submitChange(root: string, rt: Runtime = runtime): ObjectValue {
  ...
  const diff = gitDiff(root, source); requireThat(diff.trim().length > 0, 'CHANGE_MISSING', selection.source_path, 'No learner change is available to submit.');
  const manifest = encode({ task_id: task.id, base_revision: selection.source_revision, diff });
...
function assertCurrentWork(root: string, task: ObjectValue, rt: Runtime): void {
  const selection = record(root, 'current-project.yaml', rt); const source = safePath(root, selection.source_path, rt);
  requireThat(git(root, source, ['rev-parse', 'HEAD']) === selection.source_revision, 'SOURCE_REVISION_CHANGED', selection.source_path, 'Base commit changed after project selection.');
  const expected = encode({ task_id: task.id, base_revision: selection.source_revision, diff: gitDiff(root, source) });
...
export function testTask(root: string, prediction: string, rt: Runtime = runtime): ObjectValue {
  ...
  const selection = record(root, 'current-project.yaml', rt); const source = safePath(root, selection.source_path, rt);
  const args = ['-pl', 'spring-petclinic-customers-service', 'test']; const started = rt.now();
  const run = spawnSync(path.join(source, 'mvnw'), args, { cwd: source, encoding: 'utf8', timeout: 900_000, maxBuffer: 20 * 1024 * 1024 });
  const observed = { task_id: task.id, work_revision: task.work_artifact.revision, base_revision: selection.source_revision, command: `./mvnw ${args.join(' ')}`, prediction, started_at: started, completed_at: rt.now(), exit_code: run.status, signal: run.signal, stdout: run.stdout, stderr: run.stderr };
  const testArtifact = writeArtifact(root, 'test-runs', encode(observed), 'Observed focused Maven test run', rt);
...
export async function taskReview(root: string, adapter: RoleAdapter, rt: Runtime = runtime): Promise<ObjectValue> {
  ...
  if (task?.status === 'completed') {
    ...
    return { outcome: 'no-change', task_id: task.id, review_id: existing?.id ?? null, evidence_ids: task.completion_evidence_ids, next_task_preview: taskTemplate().next_task_preview };
  }
  ...
  const highest = task.assistance_history.length ? Math.max(...task.assistance_history.map((item: ObjectValue) => item.level)) : 0;
  const baseEvidence = { ...common, created_at: at, author: lead, project_id: task.project_id, task_id: task.id, scope: task.scope, difficulty: task.difficulty, assistance: { known: true, highest_level: highest, events: task.assistance_history }, verification: { status: 'verified', method: 'Team lead inspected the exact change, code review, and captured Maven result.', reviewer: lead, checked_at: at } };
  const implementation = { ...baseEvidence, id: implementationId, competency_id: 'core.implementation', fact: { observation: 'Learner submitted the reviewed PetClinic change satisfying the bounded implementation contract.', observed_at: at, artifacts: [task.work_artifact] }, interpretation: { independence: highest === 0 ? 'independent' : 'guided', direction: 'supporting', target_level: 'E1', strength: 'moderate', rationale: result.output.evidence_rationale }, supersedes: provisional.id };
  const testing = { ...baseEvidence, id: testingId, competency_id: 'core.testing', fact: { observation: 'Focused customer-service Maven tests completed successfully for the submitted work revision.', observed_at: at, artifacts: [testArtifact] }, interpretation: { independence: highest === 0 ? 'independent' : 'guided', direction: 'supporting', target_level: 'E1', strength: 'moderate', rationale: 'Observed focused tests support bounded testing behavior in one context only.' }, supersedes: null };
  task.validation = task.acceptance_criteria.map((criterion: ObjectValue) => ({ criterion_id: criterion.id, result: 'pass', artifact: criterion.id === 'AC-tests' ? testArtifact : criterion.id === 'AC-explanation' ? task.design_artifact : task.work_artifact, checked_at: at, work_revision: task.work_artifact.revision }));
  ...
  return { outcome: 'success', ...publish(root, 'team-lead', 'complete-task', task.id, { code_review_id: code.id, transcript }, writes, [implementationId, testingId, reviewId, task.id], rt), task_id: task.id, review_id: reviewId, evidence_ids: [implementationId, testingId], next_task_preview: taskTemplate().next_task_preview };
}
...
export async function performanceReview(root: string, adapter: RoleAdapter, rt: Runtime = runtime): Promise<ObjectValue> {
  ...
  if (existing) return { outcome: 'no-change', review_id: existing.id, review_outcome: existing.outcome, findings: existing.findings, next_task_preview: taskTemplate().next_task_preview };
  ...
  return { outcome: 'success', ...publish(root, 'manager', 'performance-review', profile.learner_id, { task_id: task.id, transcript }, new Map([[statePath(`reviews/performance/${id}.yaml`), encode(review)]]), [id], rt), review_id: id, review_outcome: result.output.outcome, findings: review.findings, next_task_preview: taskTemplate().next_task_preview };
}
...
export function nextAction(root: string, rt: Runtime = runtime): ObjectValue {
  ...
  if (task.status === 'completed') {
    const performance = recordsByPrefix(root, 'reviews/performance/', rt); if (!performance.length) return { phase: 'NEXT TASK / PERFORMANCE REVIEW', command: 'review performance', next_task_preview: taskTemplate().next_task_preview };
...
export async function advanceNext(root: string, adapter: RoleAdapter, rt: Runtime = runtime): Promise<ObjectValue> {
  ...
  else if (navigation.command === 'task assign pet-type-integrity') result = assignTask(root, rt);
