...
export function catalogs(): { projects: ObjectValue[]; tracks: ObjectValue[]; competencies: ObjectValue[]; competencyIds: Set<string>; coreCompetencyIds: Set<string> } {
  ...
  const competencyKeys = new Set(['id', 'domain', 'name', 'observable_behavior', 'required_core', 'prerequisites', 'encompasses']);
  requireThat(competencies.competencies.every((c: ObjectValue) => Object.keys(c).every(key => competencyKeys.has(key))), 'CATALOG_INVALID', 'catalog/competencies.yaml', 'Competency records allow only the catalog 4.0 fields.');
  ...
  validateCompetencyGraph(competencies.competencies);
