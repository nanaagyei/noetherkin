...
Claude uses safe mode, disabled slash commands, restricted file tools, no MCP servers, and no shell/network tools. Its observed startup event must contain no skills/plugins/MCP servers beyond the controlled configuration. Claude Code 2.1.280 reports two built-in plugins, `agents-md@builtin` and `telemetry@builtin`, even in safe mode; exactly these two are allowed, and only from the `builtin` path. A canary run on 2026-09-23 placed unique tokens in workspace and parent `AGENTS.md`/`CLAUDE.md` files under the evaluation flags: none reached the session, and neither did the maintainer's global instruction files. Rerun that canary after a Claude Code upgrade. The host still injects the signed-in account email as system context; it carries no instructions, but it is personal data, so treat raw transcripts as private. Codex disables host skill discovery, project instructions, hooks, memories, apps/plugins, delegation, shell, web search and browser/image tools. Its local MCP server exposes only workspace-relative list/read/write operations, rejecting absolute paths, traversal and symlinks. These three file tools are explicitly approved within their disposable workspace; no remote tools are approved. Native Codex patches remain subject to its workspace-write sandbox.
