...
The adopted proposals, alternatives, issue dispositions and migration limits are in FOUNDATION_CHANGELOG.md (repository context: `FOUNDATION_CHANGELOG.md`; Freeze decision history; adopted semantics are in the bundled architecture documents.). Detailed staleness and reconciliation semantics are in [judgment validity](judgment-validity.md). The frozen assignment fields, identity lifecycle and input bindings are normative, not implementation suggestions.
...
Validation (repository context: `evaluations/VALIDATION_REPORT.md`; Historical specification validation report, not a runtime capability.) separates executed artifact checks from future runtime and agent evaluations. Runtime initialization, transactional writer, migrations, adapters, final skill packages and educational calibration remain implementation work. This freeze authorizes no deployment, source execution, remote action or automatic implementation.
