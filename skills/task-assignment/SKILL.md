...
5. When the bound project has a curated pack, use `task assign`: for a compatible PetClinic checkout it instantiates `pet-type-integrity`, and for a forge project the next task in pack order, each as a frozen TRAINING task with its backlog-to-assigned history. Use a [proposal](references/proposals.md) for other tasks, with unresolved IDs and publication prerequisites explicit.
