...
3. If absent and initialization is requested, collect the CLI's required inputs and obtain its noninteractive proposal where available. Direct the learner to their own terminal to review initialization. Do not type consent or simulate a terminal for them. If the helper is unavailable, save a clearly incomplete initialization proposal.
4. For the supported PetClinic journey, direct the learner to run `onboard` in their terminal. The controller invokes the separately bound team lead, records an all-unassessed longitudinal baseline and scope agreement, then the onboarding coordinator publishes the profile pointer. Do not claim completion until `validate` succeeds.
