...
Once the learner has installed Noetherkin, `noetherkin` is on PATH; `noetherkin doctor` works without a workspace and reports missing prerequisites. Use explicit `--workspace` and `--json` for workspace inspection. See [CLI reference](cli.md) for exact flags and output:
