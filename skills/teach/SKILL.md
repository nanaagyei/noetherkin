...
3. After the attempt, correct only the remaining misconception. Ask for an explanation in their own words or one failure case; never write that response for them. If the first answer is fluent, ask at most two follow-ups that probe its boundary, then stop; stop at once if the learner declines. Direct help still requires attribution and a comprehension check. A passed check is at most weak evidence for `core.technical-communication`, never for the technical competency discussed.
