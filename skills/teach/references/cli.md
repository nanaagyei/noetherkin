...
The executable requires Node.js 24+. Install it from a checkout with `npm install` (the `prepare` script builds it) and `npm link`, which puts `noetherkin` on your PATH; contributors can use `npm ci` and `npm run build` and run `node dist/cli/main.js`. Noetherkin is not published to a package registry and, under ACP-012, will not be: the capabilities install into an agent, and the controller is built from source. A global install from a git URL (`npm install -g github:...`) currently fails under npm 11, because the build inside the git install runs without development dependencies. New learners start with `noetherkin setup`.
