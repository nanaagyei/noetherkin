...
Adopted on the project owner's explicit authorization, after the deferral condition was met: four forge packs and the PetClinic pack now exist, and the competency graph covers the frontend, site reliability, observability and data competencies as well as backend. Noetherkin gains a derived view of what a learner should look at next and why. **It adopts ordering and rejects scheduling.** The normative text is `docs/architecture/selection-model.md`.

What changes. `schemas/attention-advisory.schema.json` is the thirteenth schema: a closed object with `canonical: false` and no field for a percentage, mastery value, decay coefficient or due date. The view lives at `.apprenticeship/advisory/attention.yaml`. It is derived from the competency cache, the evidence the cache cites and the graph; `next` regenerates it in an active workspace, and it can be deleted at any time. Validation skips the directory, the onboarding handoff digests exclude it, and any record whose artifact URI points into it is rejected (`ADVISORY_CITED`). The frontier is every in-scope competency whose prerequisites are all demonstrated and whose own finding is `unassessed` or `developing`; contested findings form a separate remediation list and undemonstrated prerequisites are reported as blocked, never enforced. Ordering puts a contested or stale prerequisite first, then the oldest most-recent cited evidence, then the fewest distinct `project_id` contexts, and equal keys are reported as ties. A rework outcome on a task now lists the prerequisite competencies of its primary competencies to revisit. `next` shows a labeled advisory block naming the forges or curated packs that exercise the top items.

Two additions make the prohibitions mechanical rather than advisory. The writer refuses any path outside the advisory directory (`ADVISORY_CANONICAL_WRITE`). A task instantiated from a shipped template must carry exactly that template's competencies (`SCOPE_FABRICATED`), which is FR-41's guard against compression inflating a task's claimed coverage. The advisory modules are imported only by the CLI, and the ACP-013 structural guard now also fails if any other module imports them, so nothing that publishes canonical state or derives the cache can reach them.

Graph extension (ACP-013 edges, flagged for owner review): prerequisites for the four frontend competencies, `production.metrics`, `production.logging`, `production.observability` (now also requiring metrics and logging), `observability.instrumentation`, `sre.slo-error-budgets`, `sre.incident-management`, `data.sql`, `data.quality`, `data.batch-streaming`, `database.query-planning` and `python.testing`. No ID was renamed, no behavior reworded, no core flag flipped, and no core competency gained a non-core prerequisite. The catalog stays 4.0 because workspaces pin it by version string, not by digest, and edges never reach a judgment (FR-36); a 4.0 workspace therefore sees the richer graph in advisory output only. This is recorded as ADR-016.

Rejected alternatives are those in the proposal: a real spaced-repetition schedule, extending the canonical cache, computing on demand without persisting, and a target success rate. Open questions remain open: distinct `project_id` is a weak proxy for materially different contexts, and the baseline cut set is only as good as the graph. Schema impact: one new schema; FR-22 in `evaluations/validate_freeze.py` now asserts thirteen schemas and checks the new schema's 3.0 `$id` namespace. Catalog impact: edges only, within 4.0. Migration: none; a workspace that never runs `next` behaves exactly as before, and a 3.0-pinned workspace gets an edgeless view that puts every open competency on the frontier. Conformance: CF-38 to CF-41 and FR-37 to FR-41 are registered and executed offline; FR-38, CF-41 and FR-41 were confirmed non-vacuous by seeded defect. No archived byte or other frozen assertion changed.

## Phase 13 forge projects, 2026-09-23 (America/Chicago)

### ACP-015: first-party forge projects, adopted

Adopted on the project owner's explicit authorization. Noetherkin gains a second kind of project: a forge specification the learner builds from empty. The repository ships the problem, success criteria, operational requirements, a context budget and an ordered task pack, and never a solution. Forge projects carry E0 to E2; real open-source repositories remain the primary environment from E3. The contradiction resolved is the charter's "operate primarily against real open-source repositories", which left a beginner reading a stranger's large codebase before any learning and pushed that reading onto the agent. `PROJECT_CHARTER.md` section 2 and the `source/` note in section 10 are amended; the rule that inclusion is data-driven and core skills never hardcode repository behavior is retained and now enforced for forge IDs by a static guard.

What changes. `schemas/forge.schema.json` is the twelfth schema: closed, with no `repository_url`, `upstream_organization` or `contribution_readiness`. `catalog/forge/` starts at `forge_catalog_version` 1.0 with Eval Ledger, whose four-task `eval-ledger-core` pack lives in `tasks/forge/`. The catalog loader rejects an empty or missing pack, a pack directory holding anything but task JSON, a track alignment that exercises no aligned track's required competencies, and a record whose competencies differ from the union its tasks exercise. `current-project` gains `kind: forge`, binding a new or empty learner directory with a null `source_revision`; the specification is pinned at `.apprenticeship/forge/<id>.yaml` and a task's `project_id` may name it. Selection never clones, fetches or pins a commit, and refuses a directory that already holds code.

The lifecycle was generalized rather than special-cased. Task packs resolve from data for both kinds: an upstream project's `support.task_packs` names template files, and a forge pack is assigned in sequence, one task at a time. The PetClinic run command and investigation prompt moved from code into its template. Path compatibility probes are skipped for forge tasks, because the layout is the learner's design. A forge change snapshot is every non-ignored file of the learner's own Git repository rather than a diff against an upstream base, and a forge test run records the command the learner declares. No codebase map is required before the first forge task. A per-task performance review stays optional between pack tasks (FR-27).

One addition beyond the proposal text: criteria that only another person can satisfy, such as `AC-outside-check`, are marked in the pack (`attested_criteria`). The controller cannot observe that check, so task review refuses to run until the learner records an attestation against the exact work revision with `task attest`, and the criterion's validation cites that record. The attestation is the learner's attributable claim; a false one is fabricated evidence under INV-003. This keeps the proposal's decision that the outside check is enforced, not asserted, without pretending the controller verified a human.

Rejected alternatives are those in the proposal: summarization or indexing, forge projects at every level, nullable upstream fields on `project`, and shipped reference solutions. Resolutions of the open questions: Eval Ledger ships as `draft`, meaning it has not yet been built end to end against its own pack, and selecting it says so; the 15,000-token budget remains an unmeasured target to revise against ACP-016's `references_loaded` measurements; promoting a completed forge project into the upstream catalog is not provided and would need its own proposal because of the self-certification risk. Schema impact: one new schema; `apprenticeship-config` gains an optional `forge_catalog_version` pin, set to 1.0 for new workspaces; `current-project` gains the optional `kind` discriminator; `project.schema.json` is untouched. Catalog impact: new forge catalog 1.0. Migration: none; existing workspaces keep their exact meaning. Conformance: CF-42 to CF-45 and FR-42 to FR-46 are registered; CF-44 is a behavioral case not yet run live, and the rest are executed offline, including the full four-task pack. Frozen case FR-22 in `evaluations/validate_freeze.py` asserted exactly eleven schemas; as the proposal required, it now asserts twelve and checks that the forge schema carries the 3.0 `$id` namespace. No archived byte or other frozen assertion changed.
