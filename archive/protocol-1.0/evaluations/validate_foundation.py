...
def schema_for(path):
    ...
    return {'config.yaml':'apprenticeship-config', 'profile.yaml':'learner-profile',
            'current-project.yaml':'current-project', 'competencies.yaml':'competency-state'}[path.name]
...
assert competencies['catalog_version'] == levels['catalog_version'] == '1.0'
...
def inspect(value):
    ...
    if 'data_class' in value: assert value['data_class'] == 'fixture'
...
latest = {}
for assessment in sorted([r for r in records.values() if r.get('kind') in ('baseline','technical')],key=lambda r:r['created_at']):
...
assert set(cache['source_assessment_ids']) == {r['id'] for r in records.values() if r.get('kind') in ('baseline','technical')}
...
contracts = [p for p in (ROOT/'contracts').glob('*.md') if p.name != 'README.md']
...
print(f'PASS: {len(schemas)} schemas; {len(validated)} fixture documents; {len(ids)} competencies; 6 levels x 13 dimensions; 8 contracts; {negative} negative cases; fixture references, artifact links, task gates, derived cache and local Markdown links.')
print('LIMIT: artifact checks are fixture-specific. Runtime permission enforcement, migration, crash recovery and promotion execution are not implemented or tested here.')
