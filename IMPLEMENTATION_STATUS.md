...
- [x] Install and setup (WS1): `prepare` builds on install; `noetherkin setup` reports the environment and, with typed confirmation at each step, installs skills, initializes, selects a track and onboards; `doctor` gains the environment section; role hosts are auto-detected with a preflight before any consent prompt; `skills install` detects agents and supports `--global`, and refuses the checkout itself by default.
- [x] Human output and errors (WS2): compact renderers, per-command help, scoped usage errors, `WORKSPACE_NOT_INITIALIZED`, a `run` field on next actions, and forge-first project-selection candidates. `--json` shapes are unchanged except additive fields.
- [x] Agent path (WS3): noninteractive `init`, `track select` and `onboard` return a `terminal-handoff` next action; handoff digests exclude `.apprenticeship/advisory/`; the onboarding skill and the shared runtime guide describe PATH install, track-before-onboard, the handoff and forge journeys.
- [x] Forge content (WS4): Accessible Data Table, SLO Burn Report and Batch Ingest ship as `draft` with four-task packs; every pack runs the full offline journey in `tests/forge.test.ts`, and every aligned track routes to its forge and shares a required competency with it.
- [x] ACP-014 adopted (WS5, Phase 14): see `FOUNDATION_CHANGELOG.md` and `docs/architecture/selection-model.md`. CF-38 to CF-41 and FR-37 to FR-41 run offline; FR-38, CF-41 and FR-41 were confirmed non-vacuous by seeded defect. ACP-013 edges were extended for the new forges' competencies within catalog 4.0 (ADR-016).
- [x] Install smoke (WS6): `npm pack`, a global install into a scratch prefix, `noetherkin --help` from PATH, then `noetherkin setup` in a pseudo-terminal with a temporary HOME and a fake Codex. It installed 21 skills, initialized, selected `frontend-engineering` and onboarded, and printed `project select accessible-data-table ...` as the next command. That command ran as printed; `next` then assigned the first task and showed the advisory block, and `validate` passed. The smoke found that the tarball omitted `skill-pack/manifest.json`, which broke `skills install` and setup outside a checkout; that is fixed and covered by the package test.
- [x] Local verification: 175 Node tests, 21 skill bundles, foundation validation (13 schemas, 140 documents, 38 negative cases), 22 frozen conformance cases and 123 behavioral case definitions pass.

- [x] Release tarball install (ADR-017): a published GitHub release attaches the verified tarball as `noetherkin.tgz`, so `npm install -g https://github.com/nanaagyei/noetherkin/releases/latest/download/noetherkin.tgz` installs the CLI in one command. Tested locally by serving the packed tarball behind the same kind of 302 redirect GitHub uses for `latest/download` and installing into a scratch prefix: `--help` and `skills list` worked and the package was complete. The release gate was fixed along the way: `check-release.mjs` rejected `private: true`, which ACP-012 requires, so no release could have passed verification.

- [x] First automatic release, v0.1.0 (2026-09-24): merging #13 into `main` ran the Release workflow (plan, verify, publish), which tagged `v0.1.0` at `c15bbe2` and attached `noetherkin-0.1.0.tgz`, `noetherkin.tgz` and their `.sha256` files. From a clean prefix and HOME, the downloaded `noetherkin.tgz` matched its checksum, and `npm install -g https://github.com/nanaagyei/noetherkin/releases/latest/download/noetherkin.tgz` installed version 0.1.0; `--help`, `skills list` and `forges` worked. The **Release label** check passed live on #13.

Limits: the three new forges have not been built end to end by a real learner, so they ship as `draft`, and their task wording is untested against learners. The ACP-013 edges, the Phase 12 backend cut and the frontend, SRE and data additions, were approved by the project owner on 2026-09-24. They remain an authored judgment, not an empirical finding: a wrong edge distorts every advisory ordering built on it. The advisory ordering's context proxy (distinct `project_id`) is weak, as ACP-014 section 7 says. Live host skill discovery after `skills install` is unverified for every host. The git-URL global install fails under npm 11. This was reproduced with a local `git+file://` URL: npm's own preparation install inherits `--global` and links its temporary clone globally before any package script runs, so a `prepare`-script workaround was tried, found ineffective, and not shipped. The release tarball install replaces it. The install smoke used a fake Codex; no live model runs were made in this slice, and no behavioral case was rerun live. With the ChatGPT app installed, the Codex adapter prefers its bundled binary over a `codex` on PATH, so detection may report a host that is not signed in.
...
- [x] Added `ClaudeRoleAdapter` beside `CodexRoleAdapter`, sharing one prompt and closed output contract. `--role-adapter codex|claude` (default `codex`, or `NOETHERKIN_ROLE_ADAPTER`) selects it lazily, so a stale environment value cannot break commands that need no judgment. The Claude process runs in safe and restricted mode with no tools, an empty MCP configuration and no saved session.
- [x] Widened host projection from onboarding to all 21 manifest capabilities for generic, Codex and Claude Code, with byte-identical digests. Capabilities other than onboarding return `blocked` with `NO_CONTROLLER_BRIDGE` from `invoke` instead of throwing.
- [x] Added `skills list` and `skills install`: plan-then-write, no-clobber, identical-file no-op, full-install abort on any conflict, and post-write digest verification.
...
- [x] Adopted ACP-013 (Phase 12): competency catalog 4.0 with validated `prerequisites`/`encompasses` edges (known IDs, no self edges, core requires only core, acyclic union, cycle named), a first cut for core plus backend-engineering flagged for owner review, 3.0 pins still valid with no synthesized graph, `competency show`, and a static FR-36 guard confirmed non-vacuous by seeded import. 144 Node tests and 122 behavioral cases validate.
- [x] Adopted ACP-015 (Phase 13): twelfth schema `forge`, forge catalog 1.0 with Eval Ledger (`draft`) and its four-task pack, `kind: forge` bindings with no clone or commit pin, data-driven task packs replacing the hardcoded PetClinic template and Maven command, whole-tree forge snapshots, learner-declared forge test commands, `task attest` for criteria only another person can check, and a charter amendment. The full four-task forge pack runs end to end offline, and a switch to PetClinic afterwards keeps forge evidence valid. 150 Node tests and 123 behavioral cases validate; the attestation gate was confirmed non-vacuous by seeded defect. Frozen FR-22 now asserts twelve schemas, as the proposal required.
- [x] ACP-014 deferred by owner decision and recorded on the proposal.
- [x] Ran the eight Phase 12 and 13 behavioral cases live on Codex (gpt-5.6-sol) and Claude Code (claude-opus-5-5). The first runs exposed an agent treating an unchecked map as verified on request (A21), responses that held each boundary but skipped its teaching or process step (A18, A19, A20, A22), a resume bullet that called learner-written software "simulated" (P21), and a bundled runtime guide still describing PetClinic as the only publishable journey. The contracts, SKILL.md entrypoints and runtime guide now make each step explicit, in host-neutral wording guarded by a test that rejects model, vendor or host names in portable instructions. On the final bundles all sixteen executions ran cleanly. On 2026-09-24 the project owner confirmed the drafted reviews for A18 to A23 and P20 on both harnesses (fourteen passes). Both P21 runs were failed, because they ran against the superseded six-test fixture and the Codex bullet omitted the assistance level. P21 was re-run on both harnesses against the corrected fixture, and the owner confirmed both as passing. Raw runs and reviews are kept privately, not in the repository.
- [x] Allowed exactly `agents-md@builtin` and `telemetry@builtin` in the Claude eval isolation check after a canary run showed neither loads workspace, parent or user instruction files under the evaluation flags. Any other plugin still fails isolation.
- [x] Fixed the P21 fixture: its run record reported six passing tests while the supplied test file contains one. The Claude run caught the mismatch; the observation now reports one passing test.

Limits: the Claude role adapter is verified offline against a fake binary, plus one live smoke on 2026-09-23. In a scratch workspace, one onboarding baseline judgment returned the closed contract in one turn, taking 6 s and costing $0.021 with no permission denials, and the resulting workspace passed `validate`. One judgment is integration evidence, not a reliability measure. The eight new behavioral cases (P20, P21, A18 to A23) now have owner-confirmed passing reviews, one live execution per harness each (P21 from its re-run). One execution per harness is smoke evidence, not a reliability measure, and the drafts were prepared by a model from the same family as one tested harness, so owner confirmation is the independent check. Eval Ledger has not been built end to end by a real learner against its own pack, so it ships as `draft`. The ACP-013 edges were approved by the project owner on 2026-09-24. Installation into a host directory is not verified against each host's live skill discovery.
...
- [x] ACP-014 adopted in Phase 14 (2026-09-24), after four forge packs landed. ACP-013 and ACP-016 were adopted in Phase 12, ACP-015 in Phase 13.
...
- [x] GitHub Support confirmed that pull requests 1-3 are purged (owner-confirmed).
- [x] Repository visibility changed to public and the social preview image added (owner-confirmed 2026-09-24).
...
- [x] ACP-014 (frontier selection), adopted in Phase 14; see `docs/architecture/selection-model.md`.
