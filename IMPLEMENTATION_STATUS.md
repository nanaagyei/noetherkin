...
Limits: the three new forges have not been built end to end by a real learner, so they ship as `draft`, and their task wording is untested against learners. The new ACP-013 edges are a first cut awaiting owner review; a wrong edge distorts every advisory ordering built on it. The advisory ordering's context proxy (distinct `project_id`) is weak, as ACP-014 section 7 says. Live host skill discovery after `skills install` is unverified for every host. The git-URL global install fails under npm 11. This was reproduced with a local `git+file://` URL: npm's own preparation install inherits `--global` and links its temporary clone globally before any package script runs, so a `prepare`-script workaround was tried, found ineffective, and not shipped. A working one-command path would be installing a packed tarball attached to a GitHub release (tarball global installs pass the smoke test); that is a distribution decision for the owner under ACP-012. The install smoke used a fake Codex; no live model runs were made in this slice, and no behavioral case was rerun live. With the ChatGPT app installed, the Codex adapter prefers its bundled binary over a `codex` on PATH, so detection may report a host that is not signed in.
...
- [ ] Change repository visibility to public. This is an owner action and the only remaining publication step.
